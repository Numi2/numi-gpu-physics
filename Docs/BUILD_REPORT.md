...
- The README showcase now renders the source-locked Deetjen dove directly through the native Metal viewer at `1120 x 630`, 72 frames, and 24 fps. It visualizes all 144 measured surface frames, labels wingtip histories as kinematic trails rather than CFD streamlines, and charts the committed 187-sample measured/regularized-BGK/RR3 force histories. The overlay reports the passed full-window numerical gate and the still-open `68.07x` viscosity/refinement boundary together. The reproducible capture rejects a wrong size, frame count, rate, file budget, or nonidentical decoded seam before replacing the committed GIF.
