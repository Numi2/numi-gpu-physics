...
- The README showcase now renders the source-locked Deetjen dove directly through the native Metal viewer at `1120 x 630`, 72 unique frames, and 24 fps. It advances monotonically through the closest repeated source poses, zero-based surface samples 27 through 121 (`27...121 ms`, a `94 ms` interval), in a body-following frame; a visibly labeled, velocity-matched `14 ms` Hermite transition closes the presentation loop without reversing the wingbeat or claiming periodic measured data. It labels wingtip histories as kinematic trails rather than CFD streamlines and charts the committed 187-sample measured/regularized-BGK/RR3 force histories. The overlay reports the passed full-window numerical gate and the still-open `68.07x` viscosity/refinement boundary together. The reproducible capture rejects a wrong size, frame count, rate, file budget, or nonidentical 73rd-frame endpoint probe before replacing the committed GIF. The encoded seam's RGB mean absolute change is `1.71052`, only `0.681574x` the median adjacent-frame change, and the unit test separately bounds the all-vertex geometric seam inside the ordinary motion envelope.
