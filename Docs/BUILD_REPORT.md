...
All listed build, compiler, audit, and test commands passed except the archived prescribed-wing full fluid ladders, which exited nonzero at their locked scientific gates after emitting complete reports. The input preflight originally exposed the halfway-geometry limitation and now passes after the link-distance implementation. The legacy full ladders prove that corrected wall placement alone does not correct the old load bias. The promoted ladder corrects the mean loads and fails only the unchanged two-finest-grid drag-convergence gate.
...
- The targeted five-cycle 20-cell case completed in `582.12 s` with a `1.717 GB` peak memory footprint and `977 MiB` archive. It produced `(CL, CD)=(1.48928, 2.16937)`; the 16-to-20 changes are `4.420%` lift and `2.495%` drag, both below the unchanged `5%` criterion. All individual timing, periodicity, symmetry, midstroke, vortex, and mean-load gates pass. The separate input audit also passes with exact mask agreement and sub-`0.00071`-cell wall placement. Because single-grid mode assigns no verdict and 20 cells is the first grid at nominal `0.05c` thickness, a 24-cell fixed-thickness comparison remains the promotion boundary.
...
The current tests prove buildability, cross-language consistency, the independent reference algebra/convergence result, production-Metal periodic shear-wave decay and convergence, steps 1–8 population agreement, translating and oscillating planar-wall profiles and forces, topology-changing momentum closure, fixed-sphere curved-boundary drag/refinement/symmetry, isolated fixed-wing lift/drag/refinement/symmetry, prescribed-wing analytic normalization/kinematics, CPU/Metal geometry agreement, sub-cell link placement, phase capture/vortex diagnostics, live Metal command ordering, moving-wing/free-flight batch invariance, field capture, deterministic load agreement, and one-step CPU/GPU rigid-body parity. Forced channel flow remains absent. The prior interpolated full flapping ladder used the retired legacy force default; its refinement and repeatability evidence remains diagnostic, but its absolute-load verdict is not a result for the promoted estimator. The promoted ladder now validates the corrected absolute load, phase timing, periodicity, symmetry, vortex coverage, batching, and lift convergence, but quantitative acceptance remains blocked by the marginal `5.024%` drag-convergence result against the unchanged `5%` gate.
