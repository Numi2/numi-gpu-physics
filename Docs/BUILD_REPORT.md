...
All listed build, compiler, audit, and test commands passed except the archived prescribed-wing full fluid ladders, which exited nonzero at their locked scientific gates after emitting complete reports. The input preflight originally exposed the halfway-geometry limitation and now passes after the link-distance implementation. The new full ladder proves that corrected wall placement alone does not correct the absolute load bias.

Results:

- The existing 26 Swift tests passed in debug and release configurations; the added 27th phase-load-decomposition test passed its focused local run. Together they lock the 80-cell static result plus published interpolation algebra, analytic kinematics, GPU layouts, CPU/Metal input geometry/link placement, phase capture, vortex diagnostics, and component closure.
...
- The independent near-wing momentum diagnostic measured raw storage-plus-flux mean `(CL, CD)=(1.18092, 2.04933)` and optional equilibrium-reservoir-adjusted `(1.15774, 2.07231)`, while conventional and Galilean accumulation reported `(7.50904, 9.56192)` and `(7.52805, 9.48616)`. A validation-only conservative moving-domain estimator then produced `(1.18061, 2.04933)`, closing the raw budget within maximum phase residuals `0.002511/0.000247` under a `0.005` tolerance. Its mean correction relative to legacy conventional total is `(-6.32843, -7.51260)`. The fixed-mask link equation is conservative; incomplete uncover stencil accounting is the localized fault domain. Production remains unchanged.
...
The current tests prove buildability, cross-language consistency, the independent reference algebra/convergence result, production-Metal periodic shear-wave decay and convergence, steps 1–8 population agreement, translating and oscillating planar-wall profiles and forces, fixed-sphere curved-boundary drag/refinement/symmetry, isolated fixed-wing lift/drag/refinement/symmetry, prescribed-wing analytic normalization/kinematics, CPU/Metal geometry agreement, sub-cell link placement, phase capture/vortex diagnostics, live Metal command ordering, moving-wing/free-flight batch invariance, field capture, deterministic load agreement, and one-step CPU/GPU rigid-body parity. Forced channel flow remains absent. The prescribed input preflight passes and the interpolated full ladder passes repeatability, batching, vortex coverage, and numerical finest-grid-change gates, but it still fails absolute coefficients and finest peak timing. Implementation is not quantitative acceptance.
